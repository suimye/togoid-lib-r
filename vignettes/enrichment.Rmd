...
A result is an ordinary data frame carrying a `print` method, so it shows up as a
tidy table in the console and still works with dplyr, `subset()` and the rest:

```{r print}
results                              # the formatted table
print(results, n = Inf)              # every row
print(results, width = 200)          # a wide console
format(results, max_genes = 12)      # the formatted data frame itself
as.data.frame(results)               # the raw values, every column
```
...
row over several lines.

### Writing the result

```{r write}
togoid_write_enrichment(results, "enrichment.tsv")
togoid_write_enrichment(results, "enrichment.csv", sep = ",")
```
...
```{r wide}
togoid_cluster_table(results, top_n = 3, alpha = 0.05)
```
...
`togoid_plot_umap_enrichment()` picks the terms it draws with
`togoid_select_terms()`. Call it yourself with the same filters and the table
cannot disagree with the picture:

```{r selected}
selected <- togoid_select_terms(results, top_n = 3, fdr_cutoff = 0.05,
                                max_label_chars = NULL)
selected <- selected[, setdiff(names(selected), c("label", "weight"))]
togoid_write_enrichment(selected, "figure_terms.tsv")
```

`inst/examples/scRNAseq_enrichment/04_visualize_umap.R` does exactly this,
writing `<figure-name>.tsv` and `<figure-name>_by_cluster.tsv` next to every
figure.
